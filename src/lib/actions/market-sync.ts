...
import type { NormalizedOrder, RemoteListingSummary } from '@/lib/markets/types'
import type { MarketListingRow, RedesignClient } from '@/types/redesign.types'

// Pull pipelines: market → local. The initial mode also builds master
// products from remote listings (the one sanctioned direction of
// "remote writes local" — it IS the migration).
...
export async function syncProductsFromMarket(
  ...
  const { resolved, error: accountError } = await resolveMarketAccount(marketAccountId)
  if (!resolved) return { success: false, syncedCount: 0, error: accountError }

  let listings: RemoteListingSummary[]
  try {
    listings = await resolved.adapter.fetchAllListings()
  } catch (fetchError) {
        ...
        if (!masterProductId) {
          const { data: created, error: createError } = await supabase
            ...
            .insert({
              ...
              sku: remote.sku || `${resolved.account.platform.toUpperCase()}-${remote.remoteRef}`,
              brand: remote.brand,
          ...
          if (remote.stockQuantity > 0) {
            await supabase.from('stock_ledger').insert({
              ...
              reason: 'initial',
              note: `${resolved.account.name} 최초 동기화`,
            })
          }
        }
  ...
  return { success: true, syncedCount: processed, error: null }
}
...
export async function syncOrdersFromMarket(
  ...
  try {
    orders = await resolved.adapter.fetchOrders({
...
export async function syncSettlementsFromMarket(
  ...
  let processed = 0
  try {
    const settlements = await resolved.adapter.fetchSettlements({
      from: new Date(Date.now() - days * 24 * 60 * 60 * 1000),
      to: new Date(),
    })
    ...
    return { success: false, syncedCount: processed, error: message }
  }
