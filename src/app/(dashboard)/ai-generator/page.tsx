...
export default function AIGeneratorPage() {
  ...
  return (
    <>
      <Header title="판매페이지 스튜디오" subtitle="AI CREATE" />
      <div className="flex-1 overflow-y-auto p-4 pb-20 lg:p-6 lg:pb-6">
        <div className="mb-6 flex flex-col gap-4 rounded-2xl border border-primary/20 bg-gradient-to-r from-primary/10 to-violet-500/5 p-4 lg:p-5">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <Badge variant="outline" className="mb-2 bg-background/70"><Sparkles className="mr-1.5 h-3.5 w-3.5 text-primary" />조사 연동 AI 제작</Badge>
              <h2 className="text-xl font-bold">벤치마킹 근거로 내 상품의 판매 흐름을 설계하세요</h2>
              <p className="mt-1 text-sm text-muted-foreground">경쟁사의 문구나 이미지를 복제하지 않고, 선택한 원리와 내 상품 사실만 사용합니다.</p>
            </div>
            <Button variant="outline" className="shrink-0 bg-background/70" asChild>
              <Link href={selectedSessionId && selectedSessionId !== 'none' ? `/benchmarking/${selectedSessionId}` : '/benchmarking'}>
                <ArrowLeft className="mr-2 h-4 w-4" /> 조사 화면으로
              </Link>
            </Button>
          </div>
          <StudioWorkflow activeStep={generated.title ? 4 : 3} compact />
        </div>
